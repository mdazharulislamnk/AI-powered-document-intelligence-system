...
def stream_rag_response(messages: list[dict], context_chunks: list[dict], model_name: str = "gemini-flash-latest"):
    ...
    try:
        ...
        ui_name = {"gemini-3-flash-preview": "Gemini 3.0 Flash", "gemini-3.6-flash": "Gemini 3.6 Flash", "gemini-3.5-flash": "Gemini 3.5 Flash"}.get(model_name, model_name)
        yield f"\n\n**Service Unavailable**: The `{ui_name}` model is currently experiencing unusually high demand. Spikes in demand are temporary. Please wait a few moments and try your request again, or I highly recommend switching to another model (like `Gemini 3.0 Flash` or `Gemini 3.6 Flash`) in the settings menu above to continue immediately."
        ...
        if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
            ...
            yield f"\n\n**Usage Limit Reached**: The current API token quota for the `{ui_name}` model has been exhausted. Please wait{wait_time} before trying again. Alternatively, I highly recommend switching to another model (like `Gemini 3.0 Flash` or `Gemini 3.6 Flash`) in the settings menu above to continue our conversation right away."
        else:
